...
:java-version: 11
= connector-gdrive-createfolder

The project **connector-gdrive-createfolder** is a Bonita Connector for **Bonita {short-bonita-version}**  **Community** version written in `java` language.
...
[source,bash]
----
git clone https://your.github.com/connector-gdrive-createfolder.git
cd connector-gdrive-createfolder/
./mwnw package
----

The build should produce a jar archive under the `target/` folder named `connector-gdrive-createfolder-1.0-SNAPSHOT.jar`
